...
router.post('/signup', async (req: Request, res: Response, next: NextFunction) => {
  ...
  try {
    ...
    if(user) {
      ...
      const newUser = await UserModel.create({username: id, password, email});
      const code = await SecretCodeModel.create({userid: newUser._id, code: 'test'});
...
router.get('/verification/:userid/:secretCode', async (req: Request, res: Response, next: NextFunction) => {
